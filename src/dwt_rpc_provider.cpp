...
void DwtRpcProvider::Run() {
    ...
    dwt::InetAddress addr(ip, port);
    // tcpserver
    dwt::TcpServer server(&m_eventloop, addr, "dwt_rpc");
...
void DwtRpcProvider::onConnection(const dwt::TcpConnectionPtr& conn) {
    if(!conn->connected()) {
        conn->shutdown(); // 断开连接
    }
}
...
void DwtRpcProvider::onMessage(
    ...
    dwt::Buffer* buffer, dwt::Timestamp time
) {
    ...
    //         <DwtRpcProvider, const dwt::TcpConnectionPtr&, google::protobuf::Message*>
...
void DwtRpcProvider::sendRpcResponse(const dwt::TcpConnectionPtr& conn, google::protobuf::Message* response) {
    ...
    LOG_INFO("连接关闭: %s", conn->name().c_str());

    conn->shutdown();
}


DwtRpcProvider::MyClosure::MyClosure(const dwt::TcpConnectionPtr& conn, google::protobuf::Message* response)
